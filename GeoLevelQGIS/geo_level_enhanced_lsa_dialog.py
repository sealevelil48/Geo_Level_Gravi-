...
class GeoLevelEnhancedLSADialog(QDialog):
    ...
    def _run_parametric(self, fixed_points):
        ...
        result = adjuster.adjust_from_lines(self.lines, fixed_points)
        self.result = result
    ...
    def _run_conditional(self, fixed_points):
        ...
        if not loops:
            QMessageBox.information(self, "Conditional Adjustment",
                                    "No closed loops detected. "
                                    "Conditional method requires loop network.\n"
                                    "Falling back to Parametric method.")
            self._run_parametric(fixed_points)
            return
        loop_indices = [
            [self.lines.index(ln) for ln in loop.lines if ln in self.lines]
            for loop in loops
        ]
        loop_indices = [li for li in loop_indices if li]
        adjuster = ConditionalAdjuster(
            check_stability=self.chk_stability.isChecked()
        )
        result = adjuster.adjust_loops(self.lines, loop_indices, fixed_points)
        self.result = result
